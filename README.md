...
- MLE is catastrophic: **24.8% of test predictions get probability zero**, which is what
  ...
  concrete.
- Laplace "fixes" the zero but overcorrects — adding 1 to every one of 6,009 vocab words
  steals so much mass from seen bigrams that it's *5x worse* than tuned Add-k.
- **Modified Kneser-Ney now takes the top spot**, edging out single-discount KN — exactly
...
- **Stupid Backoff (214.5, normalized for fair comparison)** is solidly mid-pack here —
...
Interpolation $\lambda(w_1)=C(w_1)/(C(w_1)+k)$ share the same *shape* — both trust the
...
context gives $Z(w_1) = 1 + 0.4\times(\text{leftover unigram mass})$ — always **greater
...
despite needing no tuning at all — further evidence that data-driven weighting is a
